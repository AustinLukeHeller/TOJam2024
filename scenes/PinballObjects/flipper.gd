...
func _physics_process(delta):
	...
	if !muted and (Input.is_action_just_pressed("Left") or Input.is_action_just_pressed("Right")):
		flip_sound.play()
	if !muted and (Input.is_action_just_released("Left") or Input.is_action_just_released("Right")):
		return_sound.play()
